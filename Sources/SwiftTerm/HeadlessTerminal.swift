...
public class HeadlessTerminal : TerminalDelegate, LocalProcessDelegate {
    ...
    public init (readQueue: DispatchQueue? = nil, sendQueue: DispatchQueue? = nil, options: TerminalOptions = TerminalOptions.default, onEnd: @escaping (_ exitCode: Int32?) -> ())
    {
        self.onEnd = onEnd
        terminal = Terminal(delegate: self, options: options)
        process = LocalProcess(delegate: self, readQueue: readQueue, sendQueue: sendQueue)
    }
